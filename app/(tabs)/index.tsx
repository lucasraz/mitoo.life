...
const MOOD_OPTIONS = [
  { label: 'Todos', value: '' },
  { label: 'Gratidão', value: 'Gratidão', color: '#4CAF50' },
  { label: 'Tristeza', value: 'Tristeza', color: '#2196F3' },
  { label: 'Ansiedade', value: 'Ansiedade', color: '#FF9800' },
  { label: 'Alívio', value: 'Alívio', color: '#9C27B0' },
  { label: 'Esperança', value: 'Esperança', color: '#F44336' },
];
...
export default function FeedScreen() {
  ...
  useEffect(() => {
    if (identity && identity.seenManifesto === false) {
      // Pequeno delay para o usuário se situar antes do manifesto abrir
  ...
  const handleMitoo = async (postId: string) => {
    try {
      ...
      // Recarrega o feed para ver a contagem atualizada
      fetchPosts();
    } catch (e) {
  ...
  useEffect(() => {
    // Debounce na busca por texto para não sobrecarregar o DB
  ...
  const getGreeting = () => {
    ...
    const finalName = displayName || 'Viajante';

    if (theme.name === 'Manhã') return `Bom dia, ${finalName}`;
    if (theme.name === 'Tarde') return `Boa tarde, ${finalName}`;
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <View style={styles.header}>
        ...
        >
          ...
          <Text style={[styles.manifestoMiniBtnText, { color: theme.primary }]}>Leia e lembre-se</Text>
        </TouchableOpacity>
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={[styles.empty, { color: theme.text }]}>Ninguém desabafou ainda hoje. Seja o primeiro!</Text>
          }
      ...
      {/* Feedback de Mitoo (WOW Experience) */}
      {showMitooFeedback && (
        <MitooFeedback 
          color={theme.primary} 
...
const styles = StyleSheet.create({
  ...
  modalContent: {
    height: '80%',
    backgroundColor: '#FFF', // Será sobrescrito pelo background do theme no CommentSection
